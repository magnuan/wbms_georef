...
#define MAX_SV_MEAS  16384

typedef enum{svp_mode_none = 0, svp_mode_caris_v2=1, svp_mode_ascii_tuple=2, svp_mode_swift_svp=3, svp_mode_qps_bsvp=4} svp_mode_e;
typedef struct{
	float sv;
	float depth;
}sv_meas_t;
...
int svp_read_from_bsvp_file(const char* fname,sv_meas_t* sv_meas,const size_t max_count);
