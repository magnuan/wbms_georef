...
int write_nmea_to_buffer(double ts, output_data_t* data,uint32_t n, navdata_t posdata[NAVDATA_BUFFER_LEN],size_t pos_ix, aux_navdata_t *aux_navdata, /*OUTPUT*/char* outbuf){
    ...
    len += sprintf(&(outbuf[len]),"%1d,",aux_navdata->gps_status); 		// GPS Quality indicator
    len += sprintf(&(outbuf[len]),"%02d,",aux_navdata->sv_n); 	  		// Number of SVs in use, range from 00 through to 24+ 
    len += sprintf(&(outbuf[len]),"%3.1f,",aux_navdata->hor_accuracy); 			// HDOP 
    len += sprintf(&(outbuf[len]),"%0.3f,M,", -pos->z); 	// Orthometric height (MSL reference) 
    len += sprintf(&(outbuf[len]),"%0.3f,M,", aux_navdata->geoid_separation); 	// Geoid separation 
    if (aux_navdata->dgps_statid)
		len += sprintf(&(outbuf[len]),"%3.1f %04d",aux_navdata->dgps_latency,aux_navdata->dgps_statid); 					// Time since last DGPS update and DGPS reference station id, both blank
	else
